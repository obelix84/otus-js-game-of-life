  ...
  nextGeneration(): boolean {
    ...
    for (let i = 0; i < this._cells.length; i++) {
      ...
      for (let j = 0; j < this._cells[0].length; j++) {
        let count = 0;
        const topI = i - 1 < 0 ? this._cells.length - 1 : i - 1;
        const botI = (i + 1) % this._cells.length;
        const topJ = j - 1 < 0 ? this._cells[0].length - 1 : j - 1;
        const botJ = (j + 1) % this._cells[0].length;
        count +=
          this._cells[topI][topJ] +
          this._cells[topI][j] +
          this._cells[topI][botJ];
        count += this._cells[i][topJ] + this._cells[i][botJ];
        count +=
          this._cells[botI][topJ] +
          this._cells[botI][j] +
          this._cells[botI][botJ];
    ...
    const stop = this.stoppingRule(nextCells);
    this._cells = nextCells;
    return !stop;
  }

  stoppingRule(nextCells: number[][]): boolean {
    return nextCells.every((v, i) =>
      nextCells[i].every((v1, j) => v1 === this._cells[i][j]),
    );
  }
